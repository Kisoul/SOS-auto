  ...
  list.forEach(item => {
    ...
    portsToGenerate.forEach(({ port, tls }) => {
      ...
      const vmessBase64 = btoa(
        encodeURIComponent(jsonStr).replace(/%([0-9A-F]{2})/g, (_, p1) => String.fromCharCode('0x' + p1))
      );
...
function generateSurgeConfig(links) {
  ...
  vlessLinks.forEach((link, i) => {
    ...
    config += `${name} = vless, ${link.match(/@([^:]+):(\d+)/)?.[1] || ''}, ${link.match(/@[^:]+:(\d+)/)?.[1] || '443'}, username=${link.match(/vless:\/\/([^@]+)@/)?.[1] || ''}, tls=${link.includes('security=tls')}, ws=true, ws-path=${decodeURIComponent(link.match(/path=([^&#]+)/)?.[1] || '/')}, ws-headers=Host:${decodeURIComponent(link.match(/host=([^&#]+)/)?.[1] || '')}\n`;
  });
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
    ...
    .header h1 { font-size: 40px; font-weight: 700; letter-spacing: -0.3px; margin-bottom: 8px; line-height: 1.1; }
    ...
    @media (prefers-color-scheme: dark) {
      ...
      .list-item-label, .checkbox-label span { color: #f5f5f7; }
      .client-btn {
